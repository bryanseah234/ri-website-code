...
if(path.indexOf("%")===-1){if(/^https?\:\/\/.*$/gi.test(path)){var domainPartIndex=path.indexOf("#\/\/")+2;var slashAfterDomain=path.indexOf("index.html",domainPartIndex);if(slashAfterDomain===-1){domain=path;path="";}
else{domain=path.substr(0,slashAfterDomain);path=path.substr(slashAfterDomain);}}
action=domain+encodeURI(path)+query;}}
xmlRequest.open("POST.html",action,true);xmlRequest.setRequestHeader("Content-Type","application/x-www-form-urlencoded; charset=utf-8");xmlRequest.send(postData);return;}
